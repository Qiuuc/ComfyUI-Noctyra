...
class WatermarkRemover:
    ...
    def _load_pipeline(self) -> AutoImg2ImgPipeline:
        ...
        if self._pipeline is None:
            ...
            # Noctyra: 本地 SDXL 仅含 fp16 变体命名(diffusion_pytorch_model.fp16.safetensors)，
            # 先尝试 variant="fp16"+use_safetensors，失败再回退默认命名。
            try:
                self._pipeline = AutoImg2ImgPipeline.from_pretrained(  # type: ignore
                    self.model_id, variant="fp16", use_safetensors=True, **load_kwargs,
                )
            except Exception:
                self._pipeline = AutoImg2ImgPipeline.from_pretrained(  # type: ignore
