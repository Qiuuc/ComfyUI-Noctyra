...
# Noctyra: 提为模块常量(原为字面量)，便于离线时改指本地路径
...
class CtrlRegenEngine:
    ...
    def load(self) -> None:
        ...
        pipe.image_encoder = AutoModel.from_pretrained(DINOV2_MODEL_ID).to(self.device, dtype=self.torch_dtype)
        pipe.feature_extractor = AutoImageProcessor.from_pretrained(DINOV2_MODEL_ID)
